...
class DooModelGen{
     ...
     */
	public static function gen_mysql($comments=true, $vrules=true, $extends='DooModel', $createBase=true, $baseSuffix='Base',$useAutoload=false, $chmod=null, $path=null,$dbnamePrefix='') {
        self::genMySQL($comments, $vrules, $extends, $createBase, $baseSuffix,$useAutoload, $chmod, $path,$dbnamePrefix);
    }
     ...
     */
	public static function genMySQL($comments=true, $vrules=true, $extends='DooModel', $createBase=true, $baseSuffix='Base', $useAutoload=false, $chmod=null, $path=null,$dbnamePrefix='') {
        if($path===null){
		...
		foreach( $tables as $tbl ) {
			if(stristr($_SERVER['SERVER_SOFTWARE'], 'Win32')) {
				if(!isset($tbl['Tables_in_'.strtolower($dbname)])){
					echo $dbname;
					print_r($tbl);
					exit;
				}
			...
			if( !empty($extends) ) {
				if(!$useAutoload){
					if($createBase!=True) {
						if($extends==DooModelGen::EXTEND_MODEL || $extends==DooModelGen::EXTEND_SMARTMODEL)
							$filestr = "<?php\nDoo::loadCore('db/$extends');\n\nclass {$dbnamePrefix}$classname extends $extends{\n";
						else
							$filestr = "<?php\nDoo::loadClass('$extends');\n\nclass {$dbnamePrefix}$classname extends $extends{\n";
					}else {
						if($extends==DooModelGen::EXTEND_MODEL || $extends==DooModelGen::EXTEND_SMARTMODEL)
							$filestr = "<?php\nDoo::loadCore('db/$extends');\n\nclass {$dbnamePrefix}{$classname}{$baseSuffix} extends $extends{\n";
						else
							$filestr = "<?php\nDoo::loadClass('$extends');\n\nclass {$dbnamePrefix}{$classname}{$baseSuffix} extends $extends{\n";
					}
				}else{
				...
				if($createBase!=True)
					$filestr = "<?php\nclass {$dbnamePrefix}{$classname}{\n";
				else
					$filestr = "<?php\nclass {$dbnamePrefix}{$classname}{$baseSuffix}{\n";
			}
		...
		return \$v->validate(get_object_vars(\$this), \$this->getVRules());
			...
			if($createBase!=True) {
				if($fileManager->create($path. "{$dbnamePrefix}{$classname}.php", $filestr, 'w+')) {
				...
				if($fileManager->create($path . "/base/{$dbnamePrefix}{$classname}{$baseSuffix}.php", $filestr, 'w+')) {
					...
					$clsfile = $path. "{$dbnamePrefix}{$classname}.php";
					if(!file_exists($clsfile)) {
						if(!$useAutoload){
							$temp = lcfirst($dbnamePrefix);
							$loaderStr = "\nDoo::loadModel('{$temp}/base/{$dbnamePrefix}{$classname}{$baseSuffix}');\n";
						}
						$filestr = "<?php{$loaderStr}\nclass {$dbnamePrefix}{$classname} extends {$dbnamePrefix}{$classname}{$baseSuffix}{\n}";
