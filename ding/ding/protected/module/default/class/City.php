...
class City{
    ...
    static function cateToArr($data = array(),$pid = 0,$level = 0 ,$type='html')
    {
        foreach ($data as $ckey => $cvalue) 
        {
            if($cvalue['parentid']==$pid){
                ...
                City::$cateVal[$cvalue['id']]= array('name' => $l.$cvalue['name'],
    ...
    static public function cateToOption($id = 0,$child = true,$type = 'html',$data = array()){
        //所有分类转成按父类排序的数组
        if( empty($data)){
            $category = DBproxy::getProcedure('Manage')->setDimension(2)->getCity();
        }else{
            $category = $data;
        }        
        ...
        foreach (self::$cateVal as $key => $value) {
            ...
            if($type == 'array'){
                ...
                City::$arrchildids[] = $key;
                $option[$key]['arrchildid'] = implode(',',City::$arrchildids);
                City::$arrchildids = array();
                //找父类
                self::findArrParent($category['data'],$value['parentid']);
                $option[$key]['arrparentid'] = implode(',',City::$arrparentids);
                City::$arrparentids = array();
                //child
