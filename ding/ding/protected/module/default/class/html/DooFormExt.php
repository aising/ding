...
class DooFormExt {
    ...
    public function _addElements() {
        ...
        foreach ($this->_elements as $element => $k) {
            $div = isset($k[1]['div']) ? $k[1]['div'] : TRUE;
            $elementHtml = "";
            $elementAttributes = "";
            ...
            if (!isset($k[1]['hide-label']) || ($k[1]['hide-label'] != true)) {
                if (isset($k[1]['label']) && ($k[0] != "submit") && ($k[0] != "button") && ($k[0] != "captcha")) {
                    $labelField = $labelWrappOpen . '<label class="control-label" for="' . $element . '-element">' . $k[1]['label'] . '</label>' . $labelWrappClose;
                    $formElements[$element] .= $labelField;
                }
            }

            $idElement = str_replace(']', '', $element);
            $idElement = str_replace('[', '-', $idElement);
            ...
            switch ($k[0]) {
                ...
                case 'display':
                    ...
                    $elementHtml = $k[1]['content'];
                    break;
                // input text
                case 'text':
                    $placeholder = isset($k[1]['placeholder']) ? ' placeholder="'.$k[1]['placeholder'].'" ' : '';
                    
                    $elementHtml = $elementWrappOpen . '<input ' .$placeholder. $elementAttributes . ' type="text" name="' . $element . '" id="' . $idElement . '-element" />' . $elementWrappClose;
                    break;
                // input password
                case 'password':
                    $placeholder = isset($k[1]['placeholder']) ? ' placeholder="'.$k[1]['placeholder'].'" ' : '';
                    $elementHtml = $elementWrappOpen . '<input ' .$placeholder.$elementAttributes . ' type="password" name="' . $element . '" id="' . $idElement . '-element" />' . $elementWrappClose;
                    break;
                // submit field
                case 'submit':
                    ...
                    break;
                case 'button':
                    $type = isset($k[1]['type']) ? $k[1]['type'] : 'submit';;
                    $elementHtml = $elementWrappOpen . '<button ' . $elementAttributes . ' type="' .$type.'" name="' . $element . '">' . $k[1]['label'] . '</button>' . $elementWrappClose;
                    break;
                // hidden field
                case 'hidden':
                    ...
                    break;
                case 'file':
                    $elementHtml = $elementWrappOpen . '<input ' . $elementAttributes . ' type="file" name="' . $element . '" id="' . $idElement . '-element"/>' . $elementWrappClose;
                ...
                // select
                case 'select':
                    $elementHtml = $elementWrappOpen . '<select ' . $elementAttributes . ' name="' . $element . '" id="' . $idElement . '-element">';
                ...
                case 'textarea':
                    ...
                    $elementHtml = $elementWrappOpen . '<textarea ' . $elementAttributes . ' name="' . $element . '" id="' . $idElement . '-element">' . $text . '</textarea>' . $elementWrappClose;
                    break;
                // checkbox
                case 'checkbox':
                    $checked = (isset($elementValues[$element]) && ($elementValues[$element] == "on")) ? 'checked="checked"' : '';
                    $elementHtml = $elementWrappOpen . '<input ' . $elementAttributes . ' type="checkbox" id="' . $idElement . '-element" name="' . $element . '" ' . $checked . '/>' . $elementWrappClose;
                    break;
                ...
                case 'MultiCheckbox':
                    ...
                    if (isset($k[1]['multioptions']) && (count($k[1]['multioptions'] > 0))) {
                        foreach ($k[1]['multioptions'] as $optionValue => $optionName) {
                            ...
                            $elementHtml .= '<input type="checkbox" value="' . $optionValue . '" name="' . $element . '[]" id="' . $idElement . '-' . $optionValue . '"' . $checked . '/>';
                            $elementHtml .=  ' '.$optionName.' ';
                            //$elementHtml .= '</label>';
                        }
                    }
                ...
                case 'MultiRadio':
                    ...
                    if (isset($k[1]['multioptions']) && (count($k[1]['multioptions'] > 0))) {
                        foreach ($k[1]['multioptions'] as $optionValue => $optionName) {
                            ...
                            // add name for every checkbox
                            // $elementHtml .= '<label for="' . $element . '-' . $optionValue . '">' . $optionName;
                            // $elementHtml .= '<input type="radio" value="' . $optionValue . '" name="' . $element . '" id="' . $idElement . '-' . $optionValue . '" ' . $checked . '/>';
                            // $elementHtml .= '</label>';

                            
                            $elementHtml .= '&nbsp;&nbsp;<input ' . $elementAttributes . 'type="radio" value="' . $optionValue . '" name="' . $element . '" id="' . $idElement . '-' . $optionValue . '" ' . $checked . '/>';
                            $elementHtml .= $optionName;
            ...
            $help = isset($k[1]['help']) ? '<span class="help-block">'.$k[1]['help'].'</span>' : '';
            
            $left = isset($k[1]['left']) ? $k[1]['left'] : '';
            $right = isset($k[1]['right']) ? $k[1]['right'] : '';
            $elementHtml = $div ? '<div class="controls">'.$elementHtml.$help.'</div>' : $elementHtml.$help;
            $elementHtml = $left.$elementHtml.$right;
            // add element
            $formElements[$element] .= $div ? '<div class="control-group">' .$elementHtml . $fieldWrappClose.'</div>' : $elementHtml . $fieldWrappClose;
        }
