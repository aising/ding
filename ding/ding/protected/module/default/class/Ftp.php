...
class Ftp {
	...
	/**
	 * 生成目录，支持递归，返回FALSE时可能是目录已存在
	 * @param string $path 目录标识(ftp)
	 * @param int $permissions 文件权限列表	
	 * @return boolean 返回创建的最后一个目录状态
	 */
	 ...
	 * @return	boolean
	 */
	public function mkdir2($path = '', $permissions = NULL) {
		if($path == '' OR ! $this->_isconn()) {
			return FALSE;
		}
	...
	private function _settype($ext) {
        ...
        public function sysType(){
            ...
            if($systype==false){
                $this->_error('retrun systype fail');
            }
            return $systype;
        }
	...
	private function _error($msg) {            
		return file_put_contents('./logs/ftp_err.log', "date:[".date("Y-m-d H:i:s")."]\thostname[".$this->hostname."]\tmsg[".$msg."]\n", FILE_APPEND|LOCK_EX);
	}
